...
//      print_normeAntisismiche_token       print a normeAntisismiche token and its semantic value
//      dump_normeAntisismiche_token        dump a readable token representation
//      strdup                 duplicate a string (missing from some libraries)
//
///////////////////////////////////////////////////////////////////////////////

#include "normeAntisismiche-io.h"     // for cerr, <<, manipulators
#include <ctype.h>       // for isprint
#include "normeAntisismiche-parse.h"  // defines tokens
...
//
char *normeAntisismiche_token_to_string(int tok)
{
  switch (tok) {
  ...
  case (parete):         return("parete");		break;
  case (linea_piano):	 return("linea_piano");		break;
  case (cordolo):	 return("cordolo");		break;
  case (apertura):       return("apertura");		break;
  case (architrave):	 return("architrave");		break;
  case (float_const):    return("float_const"); 	break;
  case (ERROR_CONST):    	 return("error"); 	break;
  /*case '+': return("'+'"); break;
  case '/': return("'/'"); break;
  case '-': return("'-'"); break;
  case '*': return("'*'"); break;
...
void print_normeAntisismiche_token(int tok)
{

  cerr << normeAntisismiche_token_to_string(tok);

  switch (tok) {
/*  case (STR_CONST):
    cerr << " = ";
    cerr << " \"";
    print_escaped_string(cerr, normeAntisismiche_yylval.symbol->get_string());
    cerr << "\"";
#ifdef CHECK_TABLES
    stringtable.lookup_string(normeAntisismiche_yylval.symbol->get_string());
#endif
    break; */
/*  case (INT_CONST):
    cerr << " = " << normeAntisismiche_yylval.symbol;
#ifdef CHECK_TABLES
    inttable.lookup_string(normeAntisismiche_yylval.symbol->get_string());
#endif
    break; */
    case (float_const):
    cerr << " = " << normeAntisismiche_yylval.symbol;
#ifdef CHECK_TABLES
    floattable.lookup_string(normeAntisismiche_yylval.symbol->get_string());
#endif
    break; 

/*  case (BOOL_CONST):
    cerr << (normeAntisismiche_yylval.boolean ? " = true" : " = false");
    break; */
//  case (TYPEID): 
/*  case (OBJECTID):
    cerr << " = " << normeAntisismiche_yylval.symbol;
#ifdef CHECK_TABLES
    idtable.lookup_string(normeAntisismiche_yylval.symbol->get_string());
#endif
    break; */
/*  case (ERROR): 
    cerr << " = ";
    print_escaped_string(cerr, normeAntisismiche_yylval.error_msg);
    break; */
  }
}

// dump the token in format readable by the sceond phase token lexer
void dump_normeAntisismiche_token(ostream& out, int lineno, int token, YYSTYPE yylval) {
    out << "#" << lineno << " " << normeAntisismiche_token_to_string(token);

    switch (token) {
/*    case (STR_CONST):
	out << " \"";
	print_escaped_string(out, normeAntisismiche_yylval.symbol->get_string());
	out << "\"";
#ifdef CHECK_TABLES
	stringtable.lookup_string(normeAntisismiche_yylval.symbol->get_string());
#endif
	break; */
/*    case (INT_CONST):
	out << " " << normeAntisismiche_yylval.symbol;
#ifdef CHECK_TABLES
	inttable.lookup_string(normeAntisismiche_yylval.symbol->get_string());
#endif
	break; */
	...
	break;
/*    case (BOOL_CONST):
	out << (normeAntisismiche_yylval.boolean ? " true" : " false");
	break; */
//    case (TYPEID):
/*    case (OBJECTID):
	out << " " << normeAntisismiche_yylval.symbol;
...
/*    case (ERROR): 
        ...
        // input stream
        if (normeAntisismiche_yylval.error_msg[0] == 0) {
          out << " \"\\000\"";
        }
        else {
          out << " \"";
          print_escaped_string(out, normeAntisismiche_yylval.error_msg);
          out << "\"";
          break; 
        }*/
   }
